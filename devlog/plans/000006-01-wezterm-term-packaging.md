...
The branch should prove the smallest viable path first: direct Git-pinned upstream source with an explicit package name. A fork or artifact workflow should remain a fallback only if this path is too heavy or brittle for default development and CI.
...
2. Validate the feature-gated engine spike tests against the direct Git dependency.
